<template>
  <div class="home home2" ref="contentRef">
    <div class="headFilter" ref="headFilterRef">
      <div class="headerTop">
        ...
        <div class="rightBtns">
          ...
          <!-- <a-button class="ml10" @click="NewUser">新建</a-button> -->
        </div>
      </div>
    ...
    <common-form-modal :isShow="isCommon" v-if="isCommon" @cancel="handleCommonCancel" :title="listId?'编辑':'新建'" @success="handleSearch" :id="listId" :entityApiName="sObjectName"></common-form-modal>
    <!-- 弹窗 -->
...
<script setup>
  ...
  const { isCollapsed, tableHeight, isFilterPicker, queryParams,
     isModal, isCommon, isLock, filterList, currentFilter,
    ...
    isDeleteModal, isFilterModal, searchFilterVal, filterListFixed, entityType,
     initialData, actionList, title, sObjectName, isChartModal, menuStyle, isDefauleMneuActions, clickRecordData, listId } = toRefs(data);
  ...
  const handleEdit = () => {
    // console.log("clickRecordData", data.clickRecordData);
    data.listId = data.clickRecordData.id;
    data.isCommon = true;
    data.isDefauleMneuActions = false;
  }
  const getListConfig = () => {
...
const handleClickBtn = (devNameOrId) => {
  const func = functionsMap[devNameOrId];
  if (typeof func === "function") {
      func();
  } else {
      console.error(`Function ${devNameOrId} not found`);
  }
};
