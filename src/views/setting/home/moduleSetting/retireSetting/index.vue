          ...
          <div class="ruleItem" v-for="(item, index) in ruleList" :key="index">
              ...
              <div class="ruleItemBd">
                  ...
                  <div class="rowForm">
                      ...
                      <div class="val">
                          <a-input type="number" v-model:value="item.RetireAge"></a-input>
                      </div>
                  </div>
                  <div class="rowForm">
                  ...
                  <div v-if="item.isFilter">
                      <FilterQuery @params="(e)=>handleFilterItem(e, item)" />
                  </div>
              </div>
          </div>
...
const { ruleList } = toRefs(data);

const getRuleList = () => {
  proxy.$get(Interface.fetchList, {
      objectTypeCode: 34000,
      entityType: "H0s"
  }).then(res=>{
      console.log("res",res);
...
const handleAddRule = () => {
  data.ruleList.push({
      RetireAge: '',
      Name: '退休规则'+(data.ruleList.length+1),
      isFilter: false,
      tableData: []
  })
};
