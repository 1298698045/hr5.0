<template>
  <div class="home" ref="contentRef" v-if="!isNew && !isEdit">
  ...
  <div style="height: 100%;" ref="contentRef" v-if="isNew">
    <NewProfile @cancel="cencelNew" />
  </div>
  <div style="height: 100%;" ref="contentRef" v-if="isEdit">
    <EditProfile @cancel="cencelNew" />
  </div>
</template>
<script setup>
  import {
  ...
  import { ref, watch, reactive, toRefs, onMounted, getCurrentInstance, onUpdated, h, provide } from "vue";
  ...
  const profileId = reactive({
    id: "A00E0867-3A69-4F17-B96D-60E6ADD75DE3"
  });
  ...
  const { isCollapsed, tableHeight, fieldNames, searchTree, tabs, isFilterPicker,
    ...
    isDeleteModal, isFilterModal, searchFilterVal, filterListFixed, entityType,
     initialData, actionList, title, isNew, isEdit } = toRefs(data);
  ...
  [
    ...
    // },
    {
        ...
        formatter: function formatter(value, row) {
          var str = `
            <div class="iconBox">
              <div class="popup">
                <div class="option-item" onclick="handleEditProfile('${row.LIST_RECORD_ID}')">修改</div>
                <div class="option-item" onclick="handleTo('${row.WFRuleLogId}')">删除</div>  
                <div class="option-item" onclick="handleTo('${row.WFRuleLogId}')">查看</div>  
              </div>
    ...
    { field: 'Name', title: '简档名', sortable: true },
    { field: 'CustomProfile', title: '自定义', sortable: true,formatter:function (value) {
    ...
    formatter: function formatter(value, row) {
      ...
      { field: 'ModifiedOn', title: '最后修改时间', sortable: true, 
    formatter: function formatter(value, row) {
        var value0 = value?moment(value).format("YYYY-MM-DD HH:mm"):'';
        return value0;
    } }
