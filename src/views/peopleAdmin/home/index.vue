<template>
  <div class="home stdview-layoutWrapper" ref="contentRef">
      ...
      <div class="searchWrap">
        ...
        <div class="search-btns">
          ...
          <a-dropdown class="ml10" :trigger="['click']">
            ...
            <template #overlay>
              <a-menu class="fh-menu" style="width: 136px;" @click="handleSwitchView">
                <a-menu-item key="list">列表</a-menu-item>
                <a-menu-item key="card">卡片</a-menu-item>
                <a-menu-item key="splitScreen">分屏视图</a-menu-item>
              </a-menu>
            </template>
          </a-dropdown>
    ...
    <common-form-modal :isShow="isCommon" v-if="isCommon" @cancel="handleCommonCancel" :title="listId?'编辑':'新建'" @success="handleSearch" :id="listId" :objectTypeCode="objectTypeCode" :entityApiName="sObjectName"></common-form-modal>
    <!-- 弹窗 -->
...
<script setup>
  ...
  import PeopleCard from "@/components/peopleAdmin/PeopleCard.vue";
  ...
  const getTreeData = (keys) => {
    proxy.$get(Interface.deptTree, {}).then((res) => {
      ...
      gData.value = formTreeData(listData, 'id', 'parentId');
      gDataAll.value = formTreeData(listData, 'id', 'parentId');
    });
  }
  getTreeData();
  //树选择
    ...
    proxy.$get(Interface.listView.actions, {
      ...
      let actions = [
          [
            ...
            {
              devNameOrId:'edit',label:'编辑'
            },
            {
              devNameOrId:'BatchUpdate',label:'打印岗位说明书'
            },
            {
              devNameOrId:'ViewOrgChart',label:'删除'
            }
          ]
      ];
      if(res&&res.actions&&res.actions[0]&&res.actions[0].returnValue){
