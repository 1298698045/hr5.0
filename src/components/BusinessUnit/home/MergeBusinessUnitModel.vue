            ...
            <div class="section">
              ...
              <div class="sectionRow">
                <div class="sectionItem">
                  <a-form-item name="fromDeptId" label="被撤销部门" :rules="[{ required: true, message: '请选择' }]">
                      <a-select allowClear v-model:value="formState.fromDeptId.Id" 
                      :default-active-first-option="false" :filter-option="false" showSearch @search="(e) => {searchlookup(e,'10', 'fromDeptId');}" @dropdownVisibleChange="(e) => {searchlookup('','10', 'fromDeptId');}">
                          <template #suffixIcon></template>
                          <a-select-option v-for="(option, optionIdx) in search.fromDeptId" :key="optionIdx" :value="option.ID">{{ option.Name }}</a-select-option>
                      </a-select>
              ...
              <div class="sectionRow">
                <div class="sectionItem">
                  <a-form-item name="toDeptId" label="数据合并到" :rules="[{ required: true, message: '请选择' }]">
                      <a-select allowClear v-model:value="formState.toDeptId.Id" 
                      :default-active-first-option="false" :filter-option="false" showSearch @search="(e) => {searchlookup(e,'10', 'toDeptId');}" @dropdownVisibleChange="(e) => {searchlookup('','10', 'toDeptId');}">
                          <template #suffixIcon></template>
                          <a-select-option v-for="(option, optionIdx) in search.toDeptId" :key="optionIdx" :value="option.ID">{{ option.Name }}</a-select-option>
                      </a-select>
                      <div class="selectIcon">
                          <SearchOutlined class="ant-select-suffix" @click="handleOpenLook('10')" />
                      </div>
              ...
              <div class="sectionRow">
                <div class="sectionItem">
                  <a-form-item name="submitDate" label="撤销日期" :rules="[{ required: true, message: '请选择' }]">
