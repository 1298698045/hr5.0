    ...
    <div class="configWrap">
        <a-form>
            ...
            <a-form-item label="显示列" class="formItem">
                <div class="showList">
                    <div class="dragList">
                        <draggable class="list-group" :list="item.listColumns" group="list" itemKey="list">
                    ...
                    <template #content>
                        <a-select style="width: 200px;margin-right: 10px;">
                            <a-select-option v-for="(row,idx) in attributes" :key="idx"
                                :value="row.attributeId">{{row.label}}</a-select-option>
                        </a-select>
  ...
  import Toast from "@/utils/toast.js";
    ...
    const getFilterList = () => {
        ...
        if (item.componentType == 'tablist') {
            if (item.tabs[item.currentTab].listComponent.config && item.tabs[item.currentTab].listComponent.config.OrderExpression) {
                var OrderExpression = JSON.parse(item.tabs[item.currentTab].listComponent.config.OrderExpression);
                item.sortField = OrderExpression[0].attributeName;
                item.sortType = OrderExpression[0].sort;
            }
            if (item.tabs[item.currentTab].listComponent.config.displayColumns) {
            ...
            if (item.tabs[item.currentTab].listComponent.config.filterExpression) {
                var filterExpression = JSON.parse(item.tabs[item.currentTab].listComponent.config.filterExpression);
