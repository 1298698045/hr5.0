...
(function(global) {
    ...
    s1.subscribe = function(messageChannelToken, listener, subscriberOptions) {
        ...
        // Beta Validation
        Sfdc.assert(
            (Sfdc.isDefAndNotNull(subscriberOptions) &&
             Sfdc.isObject(subscriberOptions) &&
             Sfdc.isDefAndNotNull(subscriberOptions.scope) &&
             subscriberOptions.scope === "APPLICATION"), 
            'sforce.one.subscribe(messageChannelToken, listener, subscriberOptions) - When you call `subscribe`, include the `scope` property and set the value to "APPLICATION". ' +
            'It should look like this: `sforce.one.subscribe(<channel>, <listener>, { scope: "APPLICATION" })`.');
